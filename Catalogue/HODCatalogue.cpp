...
double cbl::catalogue::Average_c_Zehavi_2011(double x, const double logMmin, const double logsigma_c)
{
  return 0.5 * (1. + erf((log10(x) - logMmin) / logsigma_c));
}
//=============================================================================================================

double cbl::catalogue::Average_s_Zehavi_2011(double x, const double M0, const double M1, const double alpha)
{
  return 0.5 * pow((x - M0) / M1, alpha);
}
//=============================================================================================================

//Average numbers of galaxies for central and satellite galaxies Zehavi 2005

double cbl::catalogue::Average_c_Zehavi_2005(double M_h, const double M_min)
{
  double temp;
  if (M_h > M_min)
    temp = 1.;
...
double cbl::catalogue::Average_s_Zehavi_2005(double x, const double M1, const double alpha)
...
cbl::catalogue::Catalogue::Catalogue(const Catalogue halo_catalogue, const cosmology::Cosmology &cosm, const HODType HOD_Type, const double threshold, const bool substructures, vector<double> parameters)
{
  ...
  //compute minumum stellar mass, in h unit i.e. the minimum stellar mass of the galaxy sample
  // eq.5.1.6
  ...
  */
  if (parameters.size() < 1)
    parameters = {-99., -99., -99., -99., -99., -99., -99., -99., -99., -99., -99., -99., -99., -99., -99., -99.};
  ...
  if (HOD_Type == HODType::_Zehavi11_)
      ...
      for (int ii = 0; ii < 5; ii++)
	{
	  if (parameters[ii] == -99)
		...
		{
		  if (threshold == -22)
		    parameters[2] = pow(10., 13.72);
		  else if (threshold == -21.5)
		    parameters[2] = pow(10., 13.35);
		  else if (threshold == -21.)
		    parameters[2] = pow(10., 12.71);
		  else if (threshold == -20.5)
		    parameters[2] = pow(10, 11.62);
		  else if (threshold == -20.)
		    parameters[2] = pow(10., 12.35);
		  else if (threshold == -19.5)
		    parameters[2] = pow(10., 12.33);
		  else if (threshold == -19.)
		    parameters[2] = pow(10., 9.77);
		  else if (threshold == -18.5)
		    parameters[2] = pow(10., 8.99);
		  else if (threshold == -18.)
		    parameters[2] = pow(10., 9.81);
		  else
		    ErrorCBL("Not default parameters available for this threshold", "Catalogue", "HODCatalogue.cpp");
		}
		...
		{
		  if (threshold == -22)
		    parameters[3] = pow(10, 14.80);
		  else if (threshold == -21.5)
		    parameters[3] = pow(10., 14.20);
		  else if (threshold == -21.)
		    parameters[3] = pow(10., 13.76);
		  else if (threshold == -20.5)
		    parameters[3] = pow(10., 13.43);
		  else if (threshold == -20.)
		    parameters[3] = pow(10., 12.98);
		  else if (threshold == -19.5)
		    parameters[3] = pow(10., 12.75);
		  else if (threshold == -19.)
		    parameters[3] = pow(10., 12.63);
		  else if (threshold == -18.5)
		    parameters[3] = pow(10., 12.50);
		  else if (threshold == -18.)
		    parameters[3] = pow(10., 12.42);
		  else
		    ErrorCBL("Not default parameters available for this threshold", "Catalogue", "HODCatalogue.cpp");
		}
	      else
	...
	p.[15] = alpha_s: parametrization by Moster10, eq.4.3.10
      */
      ...
      if (parameters[5] == -99)
		parameters[5] = 0.0267; // norma SHMR_c
      if (parameters[6] == -99)
		parameters[6] = pow(10, 11.9347); //break SHMR_c
      if (parameters[7] == -99)
		parameters[7] = 1.0059; //beta_c SHMR_c
      if (parameters[8] == -99)
		parameters[8] = 0.5611; //gamma_c SHMR_c
      if (parameters[9] == -99)
	{
	// sigma_c, default value setted from halo mass
	//cout << par::col_green << "Using default sigma_c" << par::col_default << endl;
	//parameters[ii]=sigma_c(;
	}
      if (parameters[10] == -99)
		parameters[10] = 0.0186; // norma SHMR_s
      if (parameters[11] == -99)
		parameters[11] = pow(10, 12.1988); // break SHMR_s
      if (parameters[12] == -99)
		parameters[12] = 0.7817; // beta_s SHMR_s
      if (parameters[13] == -99)
	parameters[13] = 0.7334; // gamma_s SHMR_s
      if (parameters[14] == -99)
	...
	//
	  ...
	  // parameters[ii]=alpha_s;
	}

      //HOD parameters Zehavi11
      logMmin = parameters[0];
      logsigma_c = parameters[1];
      M0 = parameters[2];
      M1 = parameters[3];
      alpha = parameters[4];
      ...
      M1_c = parameters[6];
      beta_c = parameters[7];
      gamma_c = parameters[8];
      sigma_c = parameters[9];

      //Moster satallite
      k_s = parameters[10];
      M1_s = parameters[11];
      beta_s = parameters[12];
      gamma_s = parameters[13];
      Phi_s = parameters[14];
      alpha_s = parameters[15];
      ...
      for (int ii = 0; ii < 3; ii++)
	{
	  if (parameters[ii] == -99)
		...
		{
		  if (threshold == -22)
		    parameters[0] = pow(10, 13.91);
		  else if (threshold == -21.5)
		    parameters[0] = pow(10, 13.27);
		  else if (threshold == -21.)
		    parameters[0] = pow(10, 12.72);
		  else if (threshold == -20.5)
		    parameters[0] = pow(10, 12.30);
		  else if (threshold == -20.)
		    parameters[0] = pow(10, 12.01);
		  else if (threshold == -19.5)
		    parameters[0] = pow(10, 11.76);
		  else if (threshold == -19.)
		    parameters[0] = pow(10, 11.59);
		  else if (threshold == -18.5)
		    parameters[0] = pow(10, 11.44);
		  else if (threshold == -18.)
		    parameters[0] = pow(10, 11.27);
		  else
		    ErrorCBL("Not default parameters available for this threshold", "Catalogue", "HODCatalogue.cpp");
		}
	      else if (ii == 1)
		{
		  if (threshold == -22)
		    parameters[1] = pow(10, 14.92);
		  else if (threshold == -21.5)
		    parameters[1] = pow(10., 14.60);
		  else if (threshold == -21.)
		    parameters[1] = pow(10., 14.09);
		  else if (threshold == -20.5)
		    parameters[1] = pow(10., 13.67);
		  else if (threshold == -20.)
		    parameters[1] = pow(10., 13.42);
		  else if (threshold == -19.5)
		    parameters[1] = pow(10., 13.15);
		  else if (threshold == -19.)
		    parameters[1] = pow(10., 12.94);
		  else if (threshold == -18.5)
		    parameters[1] = pow(10., 12.77);
		  else if (threshold == -18.)
		    parameters[1] = pow(10., 12.57);
		  else
		    ErrorCBL("Not default parameters available for this threshold", "Catalogue", "HODCatalogue.cpp");
		}
	      else
      ...
      if (parameters[3] == -99)
	parameters[3] = 0.0267; //norm SHMR_c
      if (parameters[4] == -99)
	parameters[4] = pow(10, 11.9347); //break SHMR_c
      if (parameters[5] == -99)
	parameters[5] = 1.0059; //beta_c SHMR_c
      if (parameters[6] == -99)
	parameters[6] = 0.5611; //gamma_c SHMR_c
      if (parameters[7] == -99)
	{
      ...
      if (parameters[8] == -99)
	parameters[8] = 0.0186; // norma SHMR_s
      if (parameters[9] == -99)
	parameters[9] = pow(10, 12.1988); // break SHMR_s
      if (parameters[10] == -99)
	parameters[10] = 0.7817; // beta_s SHMR_s
      if (parameters[11] == -99)
	parameters[11] = 0.7334; // gamma_s SHMR_s
      if (parameters[12] == -99)
	{
		// PHI_s, default value setted from halo mass
		//cout << par::col_green << "Using default PHI_s" << par::col_default << endl;
	...
	{ // alpha_s
      ...
      M1_c = parameters[4];
      beta_c = parameters[5];
      gamma_c = parameters[6];
      sigma_c = parameters[7];
      //Moster satallite
      k_s = parameters[8];
      M1_s = parameters[9];
      beta_s = parameters[10];
      gamma_s = parameters[11];
      Phi_s = parameters[12];
      alpha_s = parameters[13];

      cout << " Chosen parameters for  HOD: " << endl;
      cout << " M_min = " << M_min << " M_sun/h" << endl;
      cout << " M1 = " << M1 << " M_sun/h" << endl;
      cout << " alpha = " << alpha << endl;
      cout << " Minimum stellar mass = " << starMass_threshold << "M_sun/h" << endl
	   << endl;
  ...
  //---------------------------------------------------------   MOSTER 2010 ----------------------------------------------------------------------------
	...
	ErrorCBL("Warning: threshold in Moster is minimum stellar mass!", "Catalogue", "HODCatalogue.cpp");
      ...
      //	if(parameter[ii]==-99){
      if (parameters[0] == -99)
	parameters[0] = 0.0297;   // 0.0267; //norm SHMR_c    //
      if (parameters[1] == -99)
	parameters[1] = pow(10., 11.9008);   // pow(10,11.9347);//break SHMR_c  //
      if (parameters[2] == -99)
	parameters[2] = 1.0757; //← w scatter// 1.0059; //beta_c SHMR_c  //
      if (parameters[3] == -99)
	parameters[3] = 0.6310; //← w scatter// 0.5611; //gamma_c SHMR_c   //
      if (parameters[4] == -99)
	{
	  //cout << par::col_green << "If sigma_c=-99 it will be setted using the halo mass..." << par::col_default << endl;
	  //parameters[ii]=sigma_c;
      ...
      if (parameters[5] == -99)
	parameters[5] = 0.0198;   // 0.0186; // norma SHMR_s  //
      if (parameters[6] == -99)
	parameters[6] = pow(10., 12.0640);    //pow(10,12.1988); // break SHMR_s //
      if (parameters[7] == -99)
	parameters[7] = 0.8097;    //0.7817; // beta_s SHMR_s //
      if (parameters[8] == -99)
	parameters[8] = 0.6910; //← w scatter//0.7334; // gamma_s SHMR_s //
      if (parameters[9] == -99)
	{ // PHI_s
	...
	{ // alpha_s
      ...
      M1_c = parameters[1];
      beta_c = parameters[2];
      gamma_c = parameters[3];
      sigma_c = parameters[4];
      //Moster satallite
      k_s = parameters[5];
      M1_s = parameters[6];
      beta_s = parameters[7];
      gamma_s = parameters[8];
      Phi_s = parameters[9];
      alpha_s = parameters[10];

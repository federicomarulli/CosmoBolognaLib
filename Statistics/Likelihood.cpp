...
cosmobl::statistics::Likelihood::Likelihood (const shared_ptr<Data> data, const shared_ptr<Model> model, const LikelihoodType likelihood_type)
{
...
cosmobl::statistics::Likelihood::Likelihood (const shared_ptr<Data> data, const shared_ptr<Model> model, const LikelihoodType likelihood_type, const LogLikelihood_function loglikelihood_function, const bool cov)
{
...
void cosmobl::statistics::Likelihood::set_data (const shared_ptr<Data> data)
{
...
{
  if (m_data->dataType() == DataType::_1D_data_){
    switch(m_likelihood_type)
    {
      case(LikelihoodType::_GaussianLikelihood_Model_):
  ...
  else if(m_data->dataType() == DataType::_2D_data_){
    switch(m_likelihood_type)
    {
      case(LikelihoodType::_GaussianLikelihood_Model_):
...
double cosmobl::statistics::Likelihood::sample_stretch_move (const int nchains, const int chain_size, const int seed)
{
  ...
  for (int n=1;n<m_chain_size;n++) {
	...
	isIncluded = accumulate(included.begin(), included.end(), 1, std::multiplies<bool>());
      ...
      double lnprob = min(0.,(m_model->npar_eff()-1)*Log(gen_z)+proposed_loglikelihood+proposed_prior-log_likelihood[i]-log_prior[i]);

      if(Log(MH_random()) < lnprob)
      {

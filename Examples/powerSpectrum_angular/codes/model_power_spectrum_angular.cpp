...
int main () {
  ...
  try {
    ...
    model_pow.set_data_model(cosmology, catalogue.Min(cbl::catalogue::Var::_Redshift_),catalogue.Max(cbl::catalogue::Var::_Redshift_), "EisensteinHu", true, 0);

    // ------------------------------------------------------------------------------------------------------------------------------------------
    // ---- insert covariance matrix, mean and std deviation of the offset and slope of the normalized redshift distribution --------------------
    // ------------------------------------------------------------------------------------------------------------------------------------------
    
    const double mean_boffset = 208.84611; 
    const double mean_bslope = -175.71603;    
    const vector<vector<double>> covM ={{565.69763066, -525.22726299},{-525.22726299, 487.73851649}};  
    const vector<double> sigma = {23.7830628, 22.08356223}; 
    double min_boffset=mean_boffset-3*sigma[0];
    double max_boffset=mean_boffset+3*sigma[0];
    double min_bslope=mean_bslope-3*sigma[1];
    double max_bslope=mean_bslope+3*sigma[1];
    ...
    cbl::statistics::PriorDistribution off_sl_prior(cbl::glob::DistributionType::_Gaussian_, {mean_boffset, mean_bslope}, covM, {min_boffset,min_bslope}, {max_boffset,max_bslope}, 668);
    
    model_pow.set_model_limber(bias_prior, off_sl_prior);
    ...
    std::vector<double> start= {1., mean_boffset, mean_bslope};

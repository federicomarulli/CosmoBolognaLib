...
MAX_CXX_VERSION = 11
IS_CXX_BELOW_MAX_VERSION = $(shell expr "$(shell expr `g++ -dumpversion`)" "<" "$(MAX_CXX_VERSION)")
CCfits_warning = compiler selected for CCfits... $(CXX_OLD)

ifeq "$(IS_CXX_BELOW_MAX_VERSION)" "1"
# C++ compiler for CCfits
  CXX_OLD = g++
else
 ifneq ($(shell which g++-10),)
   ...
   CCfits_warning = $(shell tput setaf 1)WARNING: a g++ version between 9 and 10 is required for CCfits$(shell tput sgr0)
 endif
endif
...
cleanExamples:
	...
	rm -rf $(PWD)/Examples/cosmology/results* $(PWD)/Examples/histogram/*.dat $(PWD)/Examples/statistics/output/* $(PWD)/Examples/lognormal/output/* $(PWD)/Examples/HOD/output/* $(PWD)/Examples/HOD/codes/.ipynb* $(PWD)/Examples/numberCounts/output/* $(PWD)/Examples/clustering/output/* $(PWD)/Examples/clustering/input/ran* $(PWD)/Examples/clustering/codes/.ipynb* $(PWD)/Examples/cosmicVoids/output/* $(PWD)/Examples/powerSpectrum_angular/output/*

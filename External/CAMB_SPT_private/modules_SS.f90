...
contains
  ...
  !////////////////////////////////////////////////////////////////////////
  subroutine Transfer_GetMatterPower_SPT(MT,outputPk, itf, in, minkh, dlnkh, npoints)
    Type(MatterTransferData), intent(in) :: MT
    integer, intent(in)  :: in, itf, npoints
    real, intent(in) :: minkh, dlnkh
    real, intent(out) :: outputPk(npoints,id_Pkb22)
   
    real(dl), parameter :: cllo=1.e30_dl,clhi=1.e30_dl
    real(dl) :: kh, k, h, fcb, fnu, Tk_cb, Tk_nu, Tk_tot
    ...
    do ik=1, num_k
       ...
       matp_cb(ik) = log(Tk_cb**2*k*pi*twopi*h**3*ScalarPower(k,in))
       
       if (fnu>0) then
          Tk_nu = MT%TransferData(Transfer_nu,ik,itf)
          Tk_tot = fcb*Tk_cb + fnu*Tk_nu
          matp_rest(ik) = & 
               log((Tk_tot**2-fcb**2*Tk_cb**2)*k*pi*twopi*h**3*ScalarPower(k,in))
       else
          Tk_nu = 0.
          Tk_tot = fcb*Tk_cb + fnu*Tk_nu
          matp_rest(ik) = -30.
       ...
       if (FileNames(itf) /= '') then
          if (.not. transfer_interp_matterpower ) then
             ...
             if (CP%NonLinear==NonLinear_PkSPT1loop) then
                allocate(outputPk(points,id_Pkb22,CP%InitPower%nn))
             end if
             ...
             do in = 1, CP%InitPower%nn
                ...
                call Transfer_GetMatterPower_SPT(MTrans, &
                     outputPk(:,:,in),itf,in,minkh,dlnkh,points)
  ...
  function Kernel22(r,mu,id)
    ...
    if (id==id_22dd) then
       ...
       write(*,*) "index is out of suported range"
       stop

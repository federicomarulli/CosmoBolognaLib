...
!calculate n-point roots and weight based on Gauss-Legendre method 
!added by Shun Saito
    subroutine GauLeg(x1,x2,n,x,w)
       ...
       real(dl), intent(IN) :: x1, x2
       integer, intent(IN) :: n
       real(dl), intent(OUT), dimension(n) :: x, w
       
       integer :: m, i, j
       real(dl) :: z1, z, xM, xL, pp, p3, p2, p1
       ...
       i = 1
       do while(i<=m)
          z = cos(const_pi*(dble(i)-0.25)/(dble(n)+0.5))
          ...
          do while(abs(z-z1)>EPS)
             ...
             j = 1
             !find i-th root using Newton-method
             do while(j<=n)
                p3 = p2
                p2 = p1
                p1 = ((2._dl*j-1._dl)*z*p2-(j-1._dl)*p3)/j
                
                j = j + 1
             end do
             pp = n*(z*p1-p2)/(z*z-1._dl)
             z1 = z
             z = z1 - p1/pp
          end do
          x(i) = xM - xL*z
          x(n+1-i) = xM + xL*z
          w(i) = 2._dl*xL/((1._dl-z*z)*pp*pp)
          w(n+1-i) = w(i)

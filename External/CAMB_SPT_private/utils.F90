...
module Ranges
  ...
  Type Region
     ...
     double precision delta_max, delta_min 
     !for log spacing, the non-log max and min step size
  end Type Region
...
contains
  ...
  subroutine Ranges_Getdpoints(Reg, half_ends)
    ...
    if (halfs) then
       ...
       Reg%dpoints(Reg%npoints) =  & 
            (Reg%points(Reg%npoints) - Reg%points(Reg%npoints-1))/2
    else
       Reg%dpoints(1) = (Reg%points(2) - Reg%points(1))
       Reg%dpoints(Reg%npoints) =  & 
            (Reg%points(Reg%npoints) - Reg%points(Reg%npoints-1))
    end if
  ...
  subroutine Ranges_Write(Reg) 
    ...
    do i=1,Reg%count
       AReg => Reg%R(i)
       if (AReg%IsLog) then
          Write (*,'("Range ",I3,":", 3E14.4," log")') & 
               i, AReg%Low, AReg%High, AReg%delta 
       else
          Write (*,'("Range ",I3,":", 3E14.4," linear")') & 
               i, AReg%Low, AReg%High, AReg%delta 
       end if
    end do
...
!====================================================================================
 module AMLutils
   ...
   double precision, parameter :: root2 = 1.41421356237309504880168872421d0
   double precision, parameter :: sqrt2 = root2
...
500 call MpiStop('Error creating file '//trim(aname))
  ...
  subroutine CreateOpenFile(aname, aunit,mode, append)
    ...
    if (A) then
       open(unit=aunit,file=aname,form=mode,status='unknown', err=500, & 
            position='append')
    else
       open(unit=aunit,file=aname,form=mode,status='replace', err=500)
    end if
...
!----------------------------------------------------------------------
  ...
  900 GAMMA = RES
    ...
    subroutine GetThreeJs(thrcof,l2in,l3in,m2in,m3in)
      !Recursive evaluation of 3j symbols. 
      !Does minimal error checking on input parameters.
      implicit none
      ...
      real(dl), parameter :: tiny = 1.0d-30, srtiny=1.0d-15, huge = 1.d30, & 
           srhuge = 1.d15
...
contains
  ...
  SUBROUTINE HYGFX(A,B,C,X,HF)
    ...
    !       Output:  HF --- F(a,b,c,x)
    !       Routines called:
    !            (1) GAMMAX for computing gamma function
...
115 FORMAT(1X,'Warning! You should check the accuracy')
  ...
  !////////////////////////////////////////////////////////////////////////
  !defined as Integral^x_0 t^(a-1)exp(-t)dt
  function LImcompleteGamma(a,x)
    implicit none
    double precision, intent(IN) :: a, x
    double precision :: LImcompleteGamma
    integer :: iflag
    
    LImcompleteGamma = dexp(alngam(a,iflag))*gamain(x,a,iflag)
    
  end function LImcompleteGamma
  !////////////////////////////////////////////////////////////////////////

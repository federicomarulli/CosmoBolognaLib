...
module Transfer
  ...
  integer, parameter :: id_bs2d=1, id_bs2v=2, id_b3=3
...
contains
  ...
  subroutine Transfer_GetMatterPower_SPT(MT,outputPk, & 
    ...
    if (CP%NonLinear==NonLinear_PkSPT1loop) then
       ...
       if (CP%DoNonLocalBias) then
           ...
           outputPk(:,id_Pkb22+1) = real(PkBias(:,id_bs2d))
           outputPk(:,id_Pkb22+2) = real(PkBias(:,id_bs2v))
           outputPk(:,id_Pkb22+3) = real(PkBias(:,id_b3))
       end if

       if (CP%DoNonLocalBias_A01) then
           call GetA01Corrections(khvals,npoints,A01, & 
                logkh,matp_tot,ddmatp_tot,num_k)

           outputPk(:,id_Pkb22+4:) = real(A01(:,:))
       end if
       ...
       if (CP%NonLinear_RSDmodel==NLKaiser_AB_FoG) then
          ...
          !assuming neutrino component is neglogibly small
          outputPk(:,id_Pkb22+1:id_Pkb22+id_A33) = fcb**3.*real(Ak(:,:))
          outputPk(:,id_Pkb22+id_A33+1:id_Pkb22+id_A33+id_B422) & 
               = fcb**4.*real(Bk(:,:))
       end if
    else
    ...
    do itf=1, CP%Transfer%num_redshifts
       if (FileNames(itf) /= '') then
          if (.not. transfer_interp_matterpower ) then
             ...
             if (CP%NonLinear==NonLinear_PkSPT1loop) then
                if (CP%NonLinear_RSDmodel.NE.NLKaiser_AB_FoG) then 
                   num_id = id_Pkb22
                   
                   if (CP%DoNonLocalBias) then 
                      num_id = num_id + numid_bias
                      
                      if (CP%DoNonLocalBias_A01) then
                         num_id = num_id + numid_A01
                      end if
                   end if
                      
                else
                   num_id = id_Pkb22 + id_A33 + id_B422
                end if
             else
                num_id = id_PkLin
             end if
  ...
  subroutine GetOneLoopCorrections(khvals,npoints,Pk1loop, &
    ...
    real(dl) :: muintegrand(id_b22), Pkt
    ...
    !$OMP PARALLEL DO DEFAULT(SHARED),SCHEDULE(STATIC) &
    !$OMP & PRIVATE(kh_val,logxmin,logxmax,aX,aWX, & 
       ...
       do ix = 1, numx
             ...
             do id = id_22dd, id_b22
                if (id<=id_22vv) then
                   muintegrand(id) = muintegrand(id) + &
                        aWMU(imu)*Kernel22(dexp(aX(ix)),aMU(imu),id) &
          ...
          do id = id_22dd, id_13vv
             if (id<=id_b22) then
                ...
                Pk1loop(ik,id) = Pk1loop(ik,id) + &
                     aWX(ix)*Kernel13(dexp(aX(ix)),id)*dexp(aX(ix)) & 
                     *FuncPk(kh_val*dexp(aX(ix)),alogkh,amatp_cb,addmatp_cb,num_k)
             end if
          end do
       end do
  ...
  subroutine GetBiasCorrections(khvals,npoints,Pkbias, &
    ...
    do ik = 1, npoints
       ...
       do ix = 1, numx
          ...
          PkBias(ik,id_bs2d) = PkBias(ik,id_bs2d) + aWX(ix)*x*Pktmp(id_bs2d)
          PkBias(ik,id_bs2v) = PkBias(ik,id_bs2v) + aWX(ix)*x*Pktmp(id_bs2v)
       ...
       !factor 2 comes from the half-plane 
       PkBias(ik,id_bs2d:id_bs2v) = 2.* PkBias(ik,id_bs2d:id_bs2v) & 
            * kh_val**3/(2.*pi)**2.
  ...
  subroutine GetA01Corrections(khvals,npoints,A01, &
    ...
    real(dl) :: P1, P2, P3, B12, B31, B23(numid_A01)
    integer  :: ik, ik_start, ix, imu, id
    ...
    do ik = 1, npoints
       ...
       do ix = 1, numx
          ...
          do imu = 1, nummu
             ...
             B12 = B12 + aWMU(imu)*KernelA01_B12(mu,x)*P1*P2
             B31 = B31 + aWMU(imu)*KernelA01_B31(mu,x)*P3*P1
             
             do id = id_Bb1, id_Bbs2
                B23(id) = B23(id) + aWMU(imu)*KernelA01_B23(id,mu,x)*P2*P3
             end do

          end do

          A01(ik,id_Bb1)  = A01(ik,id_Bb1)  + aWX(ix)*x*(B12+B31+B23(id_Bb1))
          A01(ik,id_Bb2)  = A01(ik,id_Bb2)  + aWX(ix)*x*B23(id_Bb2)
          A01(ik,id_Bbs2) = A01(ik,id_Bbs2) + aWX(ix)*x*B23(id_Bbs2)

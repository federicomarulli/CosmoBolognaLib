...
!!!!!!
  ...
  subroutine InitVars
    ...
    if (CP%WantTransfer) then
       do itf=1,CP%Transfer%num_redshifts
          !write(*,*) CP%Transfer%redshifts(1:CP%Transfer%num_redshifts)
  ...
  subroutine CalcScalarSources(EV,taustart)
    ...
    integer j,ind,itf
    real(dl) :: cc(24),w(EV%nvar,9), y(EV%nvar), sources(SourceNum)
    ...
    if (fixq/=0._dl) then
       ...
       do j=1,1000      
          ...
          call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,cc,w)
...
101       if (CP%WantTransfer.and.itf <= CP%Transfer%num_redshifts) then
             if (j < TimeSteps%npoints) then
                if (tauend < tautf(itf) .and.TimeSteps%points(j+1)  > tautf(itf)) then
                   ...
                   call GaugeInterface_EvolveScal(EV,tau,y,tautf(itf),tol1,ind,cc,w)

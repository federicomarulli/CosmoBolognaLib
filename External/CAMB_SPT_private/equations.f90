...
function dtauda(a)
  ...
  grhoa2=grhoa2+grhov*Funcofw(a0,2)*a2
...
!  Compute expansion rate from: grho 8*pi*rho*a**2
       ...
       ! grhov_t=grhov*a**(-1-3*w_lam)
        grhov_t=grhov*Funcofw(a,2)
        grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t

        
        gpres=(grhog_t+grhor_t)/3+grhov_t*Funcofw(a,0)
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        if (w_Perturb) then
           ...
           dgq = dgq + vq*grhov_t*(1+Funcofw(a,0))
...
if (w_Perturb) then
        if ((abs(Funcofw(a,0)+1.d0) >= 1.d-5) .and. w_Perturb) then
           ayprime(EV%w_ix)= -3*adotoa*(cs2_lam-Funcofw(a,0))*(clxq+3*adotoa*(1+Funcofw(a,0))*vq/k) &
               -(1+Funcofw(a,0))*k*vq -(1+Funcofw(a,0))*k*z -3*adotoa*adotoa*Funcofw(a,1)*vq/k
           ayprime(EV%w_ix+1) = -adotoa*(1-3*cs2_lam)*vq + k*cs2_lam*clxq/(1+Funcofw(a,0))
        else if ((abs(Funcofw(a,0)+1.d0) <= 1.d-5) .and. w_Perturb) then
            ayprime(EV%w_ix)= -3*adotoa*(cs2_lam-Funcofw(a,0))*(clxq+3*adotoa*(1+Funcofw(a,0))*vq/k) &
               -(1+Funcofw(a,0))*k*vq -(1+Funcofw(a,0))*k*z -3*adotoa*adotoa*Funcofw(a,1)*vq/k
           ayprime(EV%w_ix+1) = 0

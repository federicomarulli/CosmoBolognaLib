...
!     in one set of parameters and produdes the corresponding output. 
    ...
    program driver
        ...
        character(LEN=Ini_max_string_len) TransferFileNames(max_transfer_redshifts), &
               ...
               !added by S.S. on 2/25/13
               Pk_NLB_FileNames(max_transfer_redshifts), Bk_NLB_FileNames(max_transfer_redshifts)
       ...
       character(LEN=Ini_max_string_len) FITSfilename
        ...
        !added by S.S.
        P%DoNonLocalBias = Ini_Read_Logical('DoNonLocalBias',.false.)
        !P%DoNonLocalBias_A01 = Ini_Read_Logical('DoNonLocalBias_A01',.false.)
        P%NonLinear_RSDmodel = Ini_Read_Int('RSDmodel',LinKaiser)
...
!  Read initial parameters.
       ...
       if (P%NonLinear==NonLinear_lens .and. P%DoLensing) then
          ...
          do i=1, P%transfer%num_redshifts
             ...
             !added by S.S.
             if (P%DoNonLocalBias) then
                Pk_NLB_FileNames(i) = Ini_Read_String_Array('transfer_Pk_NLB',i)
                if (Pk_NLB_FileNames(i) == '') then
                   Pk_NLB_FileNames(i) = trim(numcat('Pk_NLB_',i))//'.dat' 
                end if
                Pk_NLB_FileNames(i) = trim(outroot)//Pk_NLB_FileNames(i)
                Bk_NLB_FileNames(i) = Ini_Read_String_Array('transfer_Bk_NLB',i)
                if (Bk_NLB_FileNames(i) == '') then
                   Bk_NLB_FileNames(i) = trim(numcat('Bk_NLB_',i))//'.dat' 
                end if
                Bk_NLB_FileNames(i) = trim(outroot)//Bk_NLB_FileNames(i)
             end if
       ...
       call CAMB_GetResults(P)
        ...
        if (P%WantTransfer .and. .not. (P%NonLinear==NonLinear_lens .and. P%DoLensing)) then
         ...
         if (P%DoNonLocalBias) then
            ...
            call Transfer_SaveMatterPower(MT,MatterPowerFileNames,Pk_NLB_FileNames,Bk_NLB_FileNames)

...
c                                                         c  
c            Time-stamp: <2016-02-26 18:51:40 ataruya>    c
c                                                         c
...
c
      ...
      real*8   pi, Lbox, zred, ss, sigmav, growth
      character infile*50
...
c     /////// Save output data ///////
c
      open(10,file='corr_pkred2.dat',status='unknown')
      open(11,file='pkcorr2_red.dat',status='unknown')
c
      do ik =1, ik_max
...
c
      ...
      integer ix, ixmax1, ixmax2
      parameter(ikmax=3000)
      parameter(ixmax1=200, ixmax2=600)
      real*8  pi
      real*8  ak(ikmax), pk(ikmax)
      real*8  pk_A11(ikmax), pk_A12(ikmax), pk_A22(ikmax)
      real*8  pk_A23(ikmax), pk_A33(ikmax)
      real*8  pk_tA11(ikmax), pk_tA12(ikmax), pk_tA22(ikmax)
      real*8  pk_tA23(ikmax), pk_tA33(ikmax)
      real*8  pk_aa11(ikmax), pk_aa12(ikmax), pk_aa22(ikmax)
      real*8  pk_aa23(ikmax), pk_aa33(ikmax)
      real*8  kmin, kmax, xmin, xmax, mumin, mumax
      real*8  k, ww(ixmax2), xx(ixmax2)
      real*8  integ_fp
      common /pk_data/ ak, pk, ik_max
      common /wave_number/  k, xmin, xmax
      common /corr_pk/ pk_A11, pk_A12, pk_A22, pk_A23, pk_A33, 
     &     pk_tA11, pk_tA12, pk_tA22, pk_tA23, pk_tA33, pk_aa11, 
     &     pk_aa12, pk_aa22, pk_aa23, pk_aa33
      pi = 4.d0 * datan(1.d0)
c     ---------------------------------------------------
c
      kmin = ak(1)
...
c
         ...
         if(k.ge.0.2) isub =0 
c
         call gauleg(log(xmin),log(xmax),xx,ww,ixmax2-isub)
c
         do ix=1, ixmax2-isub
            xx(ix)= dexp(xx(ix))
            pk_A11(ik) = pk_A11(ik)+ww(ix)*integ_fp(1,xx(ix))
            pk_A12(ik) = pk_A12(ik)+ww(ix)*integ_fp(2,xx(ix))
            pk_A22(ik) = pk_A22(ik)+ww(ix)*integ_fp(3,xx(ix))
            pk_A23(ik) = pk_A23(ik)+ww(ix)*integ_fp(4,xx(ix))
            pk_A33(ik) = pk_A33(ik)+ww(ix)*integ_fp(5,xx(ix))
c     
            pk_tA11(ik) = pk_tA11(ik)+ww(ix)*integ_fp(6,xx(ix))
            pk_tA12(ik) = pk_tA12(ik)+ww(ix)*integ_fp(7,xx(ix))
            pk_tA22(ik) = pk_tA22(ik)+ww(ix)*integ_fp(8,xx(ix))
            pk_tA23(ik) = pk_tA23(ik)+ww(ix)*integ_fp(9,xx(ix))
            pk_tA33(ik) = pk_tA33(ik)+ww(ix)*integ_fp(10,xx(ix))
c
            pk_aa11(ik) = pk_aa11(ik)+ww(ix)*integ_fp(11,xx(ix))
            pk_aa12(ik) = pk_aa12(ik)+ww(ix)*integ_fp(12,xx(ix))
            pk_aa22(ik) = pk_aa22(ik)+ww(ix)*integ_fp(13,xx(ix))
            pk_aa23(ik) = pk_aa23(ik)+ww(ix)*integ_fp(14,xx(ix))
            pk_aa33(ik) = pk_aa33(ik)+ww(ix)*integ_fp(15,xx(ix))
         enddo
c
         pk_A11(ik) = 2.d0 * pk_A11(ik) * k**3 / (2.*pi)**2
         pk_A12(ik) = 2.d0 * pk_A12(ik) * k**3 / (2.*pi)**2
         pk_A22(ik) = 2.d0 * pk_A22(ik) * k**3 / (2.*pi)**2
         pk_A23(ik) = 2.d0 * pk_A23(ik) * k**3 / (2.*pi)**2
         pk_A33(ik) = 2.d0 * pk_A33(ik) * k**3 / (2.*pi)**2
         pk_tA11(ik) = 2.d0 * pk_tA11(ik) * k**3 / (2.*pi)**2
         pk_tA12(ik) = 2.d0 * pk_tA12(ik) * k**3 / (2.*pi)**2
         pk_tA22(ik) = 2.d0 * pk_tA22(ik) * k**3 / (2.*pi)**2
         pk_tA23(ik) = 2.d0 * pk_tA23(ik) * k**3 / (2.*pi)**2
         pk_tA33(ik) = 2.d0 * pk_tA33(ik) * k**3 / (2.*pi)**2
c
         pk_aa11(ik) = 2.d0 * pk_aa11(ik) * k**3 / (2.*pi)**2
         pk_aa12(ik) = 2.d0 * pk_aa12(ik) * k**3 / (2.*pi)**2
         pk_aa22(ik) = 2.d0 * pk_aa22(ik) * k**3 / (2.*pi)**2
         pk_aa23(ik) = 2.d0 * pk_aa23(ik) * k**3 / (2.*pi)**2
         pk_aa33(ik) = 2.d0 * pk_aa33(ik) * k**3 / (2.*pi)**2
c
      write(6,'(i3,1p6e18.10)') ik,k, pk_aa11(ik),pk_aa12(ik),
     &        pk_aa22(ik),pk_aa23(ik), pk_aa33(ik)
c
 10   continue
c
      end
...
c
      ...
      real*8  integ_fp, xmin, xmax, mumin, mumax, fp_A, fp_tA, fp_aa
...
c
         do imu=1, imu_max
            if(ip.le.5) 
     &           integ_fp = integ_fp + wmu(imu) * fp_A(ip, x, mmu(imu))
            if(ip.ge.6 .and. ip.le.10) 
     &           integ_fp = integ_fp + wmu(imu) * fp_tA(ip, x, mmu(imu))
            if(ip.ge.11)
     &           integ_fp = integ_fp + wmu(imu) * fp_aa(ip, x, mmu(imu))
         enddo
...
c ******************************************************* c
c
      function  fp_A(ip, x, mu)
c
c ******************************************************* c
c
c     ip=1 for kernel of pk_A11
c     ip=2 for kernel of pk_A12
c     ip=3 for kernel of pk_A22
c     ip=4 for kernel of pk_A23
c     ip=5 for kernel of pk_A33
c
      implicit none
      integer ip
      real*8 fp_A
...
c
      call find_pk(k, pklin1)
      call find_pk(k*sqrt(1.+x*x-2.*mu*x), pklin2)
c
      fp_A = fp_A * x * pklin1 * pklin2 / (1.+x*x-2.*mu*x)**2
c
      end
c
c ******************************************************* c
c
      function  fp_tA(ip, x, mu)
c
c ******************************************************* c
c
c     ip=6 for kernel of pk_tA11
c     ip=7 for kernel of pk_tA12
c     ip=8 for kernel of pk_tA22
c     ip=9 for kernel of pk_tA23
c     ip=10 for kernel of pk_tA33
c
      implicit none
      integer ip
      real*8 fp_tA
...
c
      ...
      call find_pk(k*sqrt(1.+x*x-2.*mu*x), pklin2)
c
      fp_tA = fp_tA * x * pklin1 * pklin2 / (1.+x*x-2.*mu*x)**2
c
      end
c
c ******************************************************* c
c
      function  fp_aa(ip, x, mu)
c
c ******************************************************* c
c
c     ip=11 for kernel of pk_aa11
c     ip=12 for kernel of pk_aa12
c     ip=13 for kernel of pk_aa22
c     ip=14 for kernel of pk_aa23
c     ip=15 for kernel of pk_aa33
c
      implicit none
      integer ip
      real*8 fp_aa
...
c
      call find_pk(k, pklin1)
      call find_pk(k*x, pklin2)
c
      fp_aa = fp_aa * x * pklin1 * pklin2 / (1.+x*x-2.*mu*x)
c
      end
c
c ******************************************************* c
...
c
      ...
      real*8  akcorr(ikmax)
      real*8  pk_A11(ikmax), pk_A12(ikmax), pk_A22(ikmax)
      real*8  pk_A23(ikmax), pk_A33(ikmax), pk_tA11(ikmax)
      real*8  pk_tA12(ikmax), pk_tA22(ikmax), pk_tA23(ikmax)
      real*8  pk_tA33(ikmax), pk_aa11(ikmax), pk_aa12(ikmax)
      real*8  pk_aa22(ikmax), pk_aa23(ikmax), pk_aa33(ikmax)
      real*8  pk0corr(ikmax), pk2corr(ikmax), pk4corr(ikmax)
      real*8  pk_A1(ikmax), pk_A2(ikmax), pk_A3(ikmax)
      real*8  akEH(ikmax), pk0EH(ikmax), pk2EH(ikmax), pk4EH(ikmax)
...
ccc      open(11,file='fact.dat',status='unknown')
      open(11,file='pkstd_corr2_tree.dat',status='unknown')
c
      do ik=1, ik_max1
c
...
c
         akcorr(ik) = ak(ik)
c
         pk_A1(ik) = ff * (pk_A11(ik) + pk_tA11(ik) + pk_aa11(ik) ) +
     &        ff*ff * (pk_A12(ik) + pk_tA12(ik) + pk_aa12(ik) ) 
         pk_A2(ik) = ff*ff * (pk_A22(ik) + pk_tA22(ik) + pk_aa22(ik)) +
     &        ff*ff*ff * (pk_A23(ik) + pk_tA23(ik) + pk_aa23(ik)) 
         pk_A3(ik) = ff*ff*ff * (pk_A33(ik) + pk_tA33(ik) + pk_aa33(ik)) 
c
         pk_A1(ik) = pk_A1(ik) * dd2*dd2
         pk_A2(ik) = pk_A2(ik) * dd2*dd2
         pk_A3(ik) = pk_A3(ik) * dd2*dd2
c
...
ccc     &        fact(1,4), fact(2,4), fact(3,4), fact(4,4)
         write(11,'(1p6e18.10)') akcorr(ik), 
     &        ( pk_A11(ik) + pk_tA11(ik) + pk_aa11(ik) ) * dd2*dd2, 
     &        ( pk_A12(ik) + pk_tA12(ik) + pk_aa12(ik) ) * dd2*dd2, 
     &        ( pk_A22(ik) + pk_tA22(ik) + pk_aa22(ik) ) * dd2*dd2,
     &        ( pk_A23(ik) + pk_tA23(ik) + pk_aa23(ik) ) * dd2*dd2,
     &        ( pk_A33(ik) + pk_tA33(ik) + pk_aa33(ik) ) * dd2*dd2
c
      enddo
c
      close(11)
      write(6,*) ' Output file: pkstd_corr2_tree.dat '
c

...
c
      program fit_pkred4
c
c               Time-stamp: <2016-02-26 18:49:14 ataruya>    c
c ********************************************************** c
c
c     Reading the data of non-linear P(k) created from read_pk2.f 
c     and computing redshift P(k) of TNS model by fitting sigmav2 
c     to N-body data 
c
c     This file also reads the A and B terms for TNS model
c
...
c
      ...
      integer i_sigmav, imodel, imodel_s
      ...
      real*8  pk_lin0, pk_lin2, pk_lin4, ss0, ss2, ss4, ss6, ss8, ss10
      real*8  aks(ikmax), pk0(ikmax), pk2(ikmax), pk4(ikmax)
      real*8  pk6(ikmax), pk8(ikmax), pk10(ikmax)
...
c
      ...
      common  /pk_data/ ak1, pk_lin, pk_dd, pk_dt, pk_tt, ik_max1
     ...
     &     pk_B4
      ...
      common /pkred_data/ aks, pk0, pk2, pk4, pk6, pk8, pk10, iks_max
c     -------------------------------------------------    c
c
...
c
 8    write(6,*)
      write(6,*) ' How to specify sigma_v: '
      write(6,*) '       Use linear theory data [0]'
      write(6,*) '       Set by hand            [1]'
      write(6,*) '       Fit to N-body data     [2]'
      read(5,*)  i_sigmav
c
      write(6,*)
...
c
      write(6,*) ' Read SPT data[0] or CLA data[1] ? '
      read(5,*) imodel
      if(imodel.eq.0) 
...
c
     ...
     &     ', kmax_model=',ak1(ik_max1)
      pause
c
c     /////// Read data files (2) corrections for TNS model ///////
c
...
c
      if(i_sigmav.eq.0 .or. i_sigmav.eq.1) goto 100 
c
...
c
 100  if(imodel.eq.0)
     ...
     &     status='unknown')
c
      if(i_sigmav.eq.0) then
         call get_initial_sigmav2(sigmav2)
      elseif(i_sigmav.eq.1) then
         write(6,*) 'type sigmav2 [(Mpc/h)^2]'
         read(5,*) sigmav2
      elseif(i_sigmav.eq.2) then
         sigmav2 = a(1)**2
      endif
c
      iks_max = ik_max1
...
c
         aks(ik) = ak1(ik)
         call pk_red(ik, sigmav2, ss0, ss2, ss4, ss6, ss8, ss10)
         pk0(ik) = ss0
         pk2(ik) = ss2
         pk4(ik) = ss4
         pk6(ik) = ss6
         ...
         pk10(ik) = ss10
c
         write(11,'(1p13e18.10)') aks(ik), pk_EH0, 
     &        pk_lin0, pk0(ik), pk_EH2, pk_lin2, pk2(ik), 
     &        pk_EH4, pk_lin4, pk4(ik), pk6(ik), pk8(ik), pk10(ik)
c         
      enddo
c
      write(6,*)
...
c     
            kmin = aks(1)
            kmax = aks(iks_max)
            call calc_xi(as, xi0, xi2, xi4, xi6, xi8, xi10, is_max)
c
            do is = 1, is_max
               write(10,'(1p7e18.10)') as(is), xi0(is), xi2(is), xi4(is)
     &              , xi6(is), xi8(is), xi10(is)
            enddo
            close(10)
...
c ******************************************************* c
c
      subroutine calc_xi(as, xi0, xi2, xi4, xi6, xi8, xi10, is_max)
c
c ******************************************************* c
c
      implicit none
      ...
      real*8  pk0, pk2, pk4, pk6, pk8,  pk10
...
c
            ...
            call find_pkred(4, k, pk6)
            call find_pkred(5, k, pk8)
            call find_pkred(6, k, pk10)
c
...
c
      ...
      real*8  pk6(ikmax), pk8(ikmax), pk10(ikmax)
      real*8  kk, s, ds, pk
      common /pkred_data/ ak, pk0, pk2, pk4, pk6, pk8, pk10, ik_max
c     -------------------------------------------
c
...
c
     ...
     &     call polint(ak(jmin),pk4(jmin),jmax-jmin+1,kk,s,ds)
      if(ilabel.eq.4) 
     &     call polint(ak(jmin),pk6(jmin),jmax-jmin+1,kk,s,ds)
      if(ilabel.eq.5) 
     &     call polint(ak(jmin),pk8(jmin),jmax-jmin+1,kk,s,ds)
      if(ilabel.eq.6) 
     &     call polint(ak(jmin),pk10(jmin),jmax-jmin+1,kk,s,ds)
c
      pk = s
c      
      end
...
c ******************************************************* c
c
      subroutine pk_red(ik, sigmav2, pk0, pk2, pk4, pk6, pk8, pk10)
c
c ******************************************************* c
c
c     imodel_s = 1 :   TNS with Gaussian damping function
c     imodel_s = 2 :   TNS with Lorentzian damping function
c
      implicit none
      integer ik, ikmax, ik_max, imodel_s
      real*8 sigmav2, pk0, pk2, pk4, pk6, pk8, pk10
      ...
      real*8  pk_A1(ikmax), pk_A2(ikmax), pk_A3(ikmax)
      real*8  pk_B1(ikmax), pk_B2(ikmax), pk_B3(ikmax), pk_B4(ikmax)
      real*8  Omega_b, Omega_m, Omega_v, h, Tcmb, n_s, w_de, zred
      common  /cosm_param/ Omega_b, Omega_m, h, Tcmb, n_s, w_de
      common  /redshift/ zred
      common  /pk_data/ ak, pk_lin, pk_dd, pk_dt, pk_tt, ik_max
      common  /pk_corr_data/ pk_A1, pk_A2, pk_A3, pk_B1, pk_B2, pk_B3, 
     &     pk_B4
      common  /model_s/ imodel_s
...
c
      Omega_v = 1.d0 - Omega_m
c
      ff = f(zred, Omega_m, Omega_v, w_de)
...
c
     ...
     &        ( 2.d0*ff*pk_dt(ik) + pk_A1(ik) + pk_B1(ik) )
     &        + fact(2,0) * ( ff*ff*pk_tt(ik) + pk_A2(ik) + pk_B2(ik) )
     &        + fact(3,0) * ( pk_A3(ik) + pk_B3(ik) )
     &        + fact(4,0) * pk_B4(ik)
c
         pk2 = fact(0,2) * pk_dd(ik) + fact(1,2) * 
     &        ( 2.d0*ff*pk_dt(ik) + pk_A1(ik) + pk_B1(ik) )
     &        + fact(2,2) * ( ff*ff*pk_tt(ik) + pk_A2(ik) + pk_B2(ik) )
     &        + fact(3,2) * ( pk_A3(ik) + pk_B3(ik) )
     &        + fact(4,2) * pk_B4(ik)
c
         pk4 = fact(0,4) * pk_dd(ik) + fact(1,4) * 
     &        ( 2.d0*ff*pk_dt(ik) + pk_A1(ik) + pk_B1(ik) )
     &        + fact(2,4) * ( ff*ff*pk_tt(ik) + pk_A2(ik) + pk_B2(ik) )
     &        + fact(3,4) * ( pk_A3(ik) + pk_B3(ik) )
     &        + fact(4,4) * pk_B4(ik)
c
         pk6 = fact(0,6) * pk_dd(ik) + fact(1,6) * 
     &        ( 2.d0*ff*pk_dt(ik) + pk_A1(ik) + pk_B1(ik) )
     &        + fact(2,6) * ( ff*ff*pk_tt(ik) + pk_A2(ik) + pk_B2(ik) )
     &        + fact(3,6) * ( pk_A3(ik) + pk_B3(ik) )
     &        + fact(4,6) * pk_B4(ik)
c
         pk8 = fact(0,8) * pk_dd(ik) + fact(1,8) * 
     &        ( 2.d0*ff*pk_dt(ik) + pk_A1(ik) + pk_B1(ik) )
     &        + fact(2,8) * ( ff*ff*pk_tt(ik) + pk_A2(ik) + pk_B2(ik) )
     &        + fact(3,8) * ( pk_A3(ik) + pk_B3(ik) )
     &        + fact(4,8) * pk_B4(ik)
c
         pk10 = fact(0,10) * pk_dd(ik) + fact(1,10) * 
     &        ( 2.d0*ff*pk_dt(ik) + pk_A1(ik) + pk_B1(ik) )
     &        + fact(2,10) * ( ff*ff*pk_tt(ik) + pk_A2(ik) + pk_B2(ik) )
     &        + fact(3,10) * ( pk_A3(ik) + pk_B3(ik) )
     &        + fact(4,10) * pk_B4(ik)
c
...
c
      ...
      real*8  zred, Omega_m, Omega_b, h, Tcmb, n_s, w_de
...
c
     ...
     &        + fact(1,0) * ( 2.d0*ff*pk_dt + pk_A1 + pk_B1 )
     &        + fact(2,0) * ( ff*ff*pk_tt + pk_A2 + pk_B2 ) 
     &        + fact(3,0) * ( pk_A3 + pk_B3 ) 
     &        + fact(4,0) * pk_B4 
            pk_red2 = fact(0,2) * pk_dd 
     &        + fact(1,2) * ( 2.d0*ff*pk_dt + pk_A1 + pk_B1 )
     &        + fact(2,2) * ( ff*ff*pk_tt + pk_A2 + pk_B2 ) 
     &        + fact(3,2) * ( pk_A3 + pk_B3 ) 
     &        + fact(4,2) * pk_B4 
         elseif(imodel_s.eq.2) then
            write(6,*) 'Sorry !!  Not yet available' 
            stop
...
c ******************************************************* c
c
      subroutine get_initial_sigmav2(sigmav2)
c
c ******************************************************* c
c
      implicit none
      ...
      parameter(ikmax=5000)
      real*8  sigmav2, pi
      real*8  ak1(ikmax), pk_lin(ikmax)
      real*8  pk_dd(ikmax), pk_dt(ikmax), pk_tt(ikmax)
      common  /pk_data/ ak1, pk_lin, pk_dd, pk_dt, pk_tt, ik_max1
c     ---------------------------------------------------------
      pi = 4.d0 * datan(1.d0)
c
cc      sigmav2 = pk_tt(1) * ak1(1) / 2.d0
      sigmav2 = pk_lin(1) * ak1(1) / 2.d0
c
      do ik = 1, ik_max1
cc         sigmav2 = sigmav2 + (pk_tt(ik+1) + pk_tt(ik)) * 
         sigmav2 = sigmav2 + (pk_lin(ik+1) + pk_lin(ik)) * 
     &        (ak1(ik+1)-ak1(ik)) /2.d0
      enddo
c
      sigmav2 = sigmav2 / (2.d0*pi*pi) / 3.d0
c
      end
c
c ******************************************************* c

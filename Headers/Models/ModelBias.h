...
namespace cosmobl{
  ...
  class ModelBias : public statistics::Model1D
  {
    public:
      ...
      void set_wp_parameters(const vector<double> r, const shared_ptr<Cosmology> cosmology, const double redshift, const double pi_max, const string type="Poly", const int nPt=4, const string method="CAMB", const string output_root="test", const int norm=-1, const double r_min=1.e-3, const double r_max=350., const double k_min=0., const double k_max=100., const double aa=0., const bool GSL=1, const double prec=1.e-2, const string file_par=par::defaultString);
	...
	return m_model(xx,m_model_parameters,parameters);
      ...
      double operator() (double xx, vector<double> parameters){
	return m_model(xx,m_model_parameters,parameters);
      }
      */
  };

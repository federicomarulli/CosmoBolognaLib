...
namespace cbl {
  ...
  namespace modelling {
	 ...
	 *
	 *  @param z_bias_prior prior on the redshif bias derived from observations,
	 *  governing the position of the mean of the Gaussian \f$ P(z_{\rm ob}|z_{\rm tr}) \f$.
	 *  In particular, the corrected redshift, \f$z_{\rm corr}\f$, is expressed as
	 *  \f$z_{\rm corr} = z - \Delta z (1+z)\f$, where \f$z\f$ is the
	 *  not corrected redshift, and \f$\Delta z\f$ is the redshift bias
	 *
	 ...
	 *  
	 */
	void set_model_NumberCounts_cosmology (const std::vector<cbl::cosmology::CosmologicalParameter> cosmo_param, const std::vector<statistics::PriorDistribution> cosmo_param_prior, const statistics::PriorDistribution alpha_prior, const statistics::PriorDistribution beta_prior, const statistics::PriorDistribution gamma_prior, const statistics::PriorDistribution scatter0_prior, const statistics::PriorDistribution scatterM_prior, const statistics::PriorDistribution scatterM_exponent_prior, const statistics::PriorDistribution scatterz_prior, const statistics::PriorDistribution scatterz_exponent_prior, const statistics::PriorDistribution z_bias_prior, const std::vector<statistics::PriorDistribution> Plambda_prior);

...
namespace cbl {
  ...
  namespace classfunc {
	...
	double Plin_BAO = cosm.Pk(kk, method_PkC, NL, m_redshift, m_output_root, norm, m_kmax)-cosm.Pk(kk, method_PkEH, NL, m_redshift);
	...
	if (m_method_Pk=="EisensteinHu") 
	  return pow(cosm.linear_growth_rate(m_redshift, kk),2)*cosm.Pk(kk, m_method_Pk, false, m_redshift)*pow(cbl::TopHat_WF(kk*m_rr),2);   

	else return cbl::ErrorCBL("Error in func_V2 of CosmClassFunc.h!"); 
      }
    };
	...
	if (m_method_Pk=="EisensteinHu") 
	  return pow(cosm.linear_growth_rate(m_redshift, kk),2)*cosm.Pk(kk, m_method_Pk, false, m_redshift)*(1.-pow(cbl::TopHat_WF(kk*m_rr),2));
	
	else return cbl::ErrorCBL("Error in func_sigma2 of CosmClassFunc.h!"); 
      }
    };
    ...
    private:
      ...
      std::string m_model_MF, m_method_SS, m_output_root;
      double m_Delta;
	...
	double n_halo_expected = cosm.n_haloes(Mmin, m_Mmax, m_z_min, m_z_max, m_angle_rad, m_model_MF, m_method_SS, m_output_root, m_Delta, m_interpType, m_kmax, m_input_file, m_is_parameter_file)*m_Area;

...
namespace cbl {
  ...
  namespace classfunc {
	...
	double Plin_BAO = cosm.Pk(kk, method_PkC, NL, m_redshift, m_store_output_CAMB, m_output_root, norm, m_kmax)-cosm.Pk(kk, method_PkEH, NL, m_redshift, m_store_output_CAMB);
	...
	if (m_method_Pk=="EisensteinHu") 
	  return pow(cosm.linear_growth_rate(m_redshift, kk),2)*cosm.Pk(kk, m_method_Pk, false, m_redshift, m_store_output_CAMB)*pow(cbl::TopHat_WF(kk*m_rr),2);   
	...
	if (m_method_Pk=="EisensteinHu") 
	  return pow(cosm.linear_growth_rate(m_redshift, kk),2)*cosm.Pk(kk, m_method_Pk, false, m_redshift, m_store_output_CAMB)*(1.-pow(cbl::TopHat_WF(kk*m_rr),2));
    ...
    public:
      func_MhaloMin (double Omega_matter, double Omega_baryon, double Omega_neutrinos, double massless_neutrinos, int massive_neutrinos, double Omega_DE, double Omega_radiation, double hh, double scalar_amp, double scalar_pivot, double n_spec, double w0, double wa, double fNL, int type_NG, double tau, std::string model, bool unit, double n_halo, double Area, bool angle_rad, double z_min, double z_max, double Mmax, std::string model_MF, std::string method_SS, bool store_output_CAMB, std::string output_root, const double Delta, std::string interpType, double kmax, std::string input_file, bool is_parameter_file)  
	: m_Omega_matter(Omega_matter), m_Omega_baryon(Omega_baryon), m_Omega_neutrinos(Omega_neutrinos), m_massless_neutrinos(massless_neutrinos), m_massive_neutrinos(massive_neutrinos), m_Omega_DE(Omega_DE), m_Omega_radiation(Omega_radiation), m_hh(hh), m_scalar_amp(scalar_amp), m_scalar_pivot(scalar_pivot), m_n_spec(n_spec), m_w0(w0), m_wa(wa), m_fNL(fNL), m_type_NG(type_NG), m_tau(tau), m_model(model), m_unit(unit), m_angle_rad(angle_rad), m_n_halo(n_halo), m_Mmax(Mmax), m_z_min(z_min), m_z_max(z_max), m_Area(Area), m_model_MF(model_MF), m_method_SS(method_SS), m_store_output_CAMB(store_output_CAMB), m_output_root(output_root), m_Delta(Delta), m_interpType(interpType), m_kmax(kmax), m_input_file(input_file), m_is_parameter_file(is_parameter_file) {}
	...
	double n_halo_expected = cosm.n_haloes(Mmin, m_Mmax, m_z_min, m_z_max, m_angle_rad, m_model_MF, m_method_SS, m_store_output_CAMB, m_output_root, m_Delta, m_interpType, m_kmax, m_input_file, m_is_parameter_file)*m_Area;

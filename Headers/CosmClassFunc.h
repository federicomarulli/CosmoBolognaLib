...
namespace cbl {
  ...
  namespace classfunc {
	...
	double Plin_BAO = cosm.Pk(kk, method_PkC, NL, m_redshift, m_store_output, m_output_root, norm, m_kmax)-cosm.Pk(kk, method_PkEH, NL, m_redshift, m_store_output);
	...
	if (m_method_Pk=="EisensteinHu") 
	  return pow(cosm.linear_growth_rate(m_redshift, kk),2)*cosm.Pk(kk, m_method_Pk, false, m_redshift, m_store_output)*pow(cbl::TopHat_WF(kk*m_rr),2);   
	...
	if (m_method_Pk=="EisensteinHu") 
	  return pow(cosm.linear_growth_rate(m_redshift, kk),2)*cosm.Pk(kk, m_method_Pk, false, m_redshift, m_store_output)*(1.-pow(cbl::TopHat_WF(kk*m_rr),2));

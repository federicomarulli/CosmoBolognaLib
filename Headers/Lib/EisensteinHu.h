...
/**
 *  @file Headers/Lib/EisensteinHu.h
 *
...
namespace cosmobl {
  ...
  namespace cosmology {
     ...
     *  cosmologies
     *  
     *  The \e eisensteinhu namespace contains all the functions and
     *  classes used for for fitting formulae for CDM + Baryon +
     *  Massive Neutrino (MDM) cosmologies
     */
    namespace eisensteinhu {

      /**
       *  @class EisensteinHu EisensteinHu.h
       *  "Headers/Lib/EisensteinHu.h"
       *
       *  @brief The class EisensteinHu
       *
	...
	public:
	   ...
	   * critical
	   *
	   * @param omega_hdm density of massive neutrinos, in units
	   * of critical
	   *
	  ...
	  int TFmdm_set_cosm (double omega_matter, double omega_baryon, double omega_hdm, int degen_hdm, double omega_lambda, double hubble, double redshift, double As=2.56e-9, double k_pivot=0.05, double n_spec=0.96)
	  {
	    ...
	    growth_to_z0 = growth_k0/growth_to_z0;	
	    double growth_z = 5.*omega_matter_z/(2*(1+redshift))/(1./70.+209./140.*omega_matter_z-pow(omega_matter_z,2)/140.+pow(omega_matter_z,4./7.));
	    */
	  ...
	  /* Output: The following are set as global variables:
	     ...
	     CDM + Baryon perturbations.  growth_cbnu -- the transfer
	     function for density-weighted CDM + Baryon + Massive
	  ...
	  /**
	   * @brief Given a wavenumber in h Mpc^-1, return the
	   * transfer function
	   *
	   ...
	   * h^{-3}Mpc^{-3}
	   */
	  vector<double> Pk(vector<double> kk) {
	    
	    vector<double> _Pk(kk.size(), 0);

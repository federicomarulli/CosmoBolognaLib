...
namespace cosmobl {
  ...
  /**
   *  @brief check if the dimension of a vector is higher than an
   *  input value
   *  @param vect a vector
  ...
  void find_index (vector<double>, double, double, int &, int &);
   ...
   */
  template <typename T> T distance (T d1, T d2, T d3)
    {
   ...
   */
  template <typename T> T angsep_xyz (T d1, T d2, T d3)
    {
      return 2.*asin(0.5*distance(d1, d2, d3));
    }
   ...
   */
  template <typename T> T angsep_haversine (T ra1, T ra2, T dec1, T dec2)
    {
      return 2*asin(pow(pow(sin((dec1-dec2)/2.),2)+cos(dec1)*cos(dec2)*sqrt(sin((ra1-ra2)/2.),2)));
    }

  /**
   *  @brief the Vincenty's angular separation in 3D
   *  @author Alfonso Veropalumbo
   *  @author alfonso.veropalumbo@unibo.it
   *  @param ra1 the Right Ascension of the first object
   *  @param ra2 the Right Ascension of the second object
   *  @param dec1 the Declination of the first object
   *  @param dec2 the Declination of the second object
   *  @return the Vincenty's angular separation
   */
  template <typename T> T angsep_vincenty (T ra1, T ra2, T dec1, T dec2)
    {
      T deltara = ra1-ra2;
      T num = pow(pow(cos(dec1)*sin(deltara),2)+pow(cos(dec2)*sin(dec1)-sin(dec2)*cos(dec1)*cos(deltara),2),0.5);
      T den = cos(ang_sep(ra1,ra2,dec1,dec2));
  ...
  void eq2sdss (vector<double> &, vector<double> &, vector<double> &, vector<double> &); 
  void sdss2eq (vector<double> &, vector<double> &, vector<double> &, vector<double> &);
  void sdss_stripe (vector<double> &, vector<double> &, vector<int> &, vector<int> &);
   ...
   *  @return &xi;(s)/&xi;(r)
   */
  double xi_ratio (double, void *, vector<double>); // for the chi2 function
  /// @endcond
   ...
   */
  double multipoles (double, void *, vector<double>);
  /// @endcond
   ...
   *  @return the multipole &xi;<SUB>0</SUB>
   */
  double multipole_xi0_model (double, void *, vector<double>);
  /// @endcond
  ...
  double xi2D_lin_model (double, double, void *, vector<double>);
  /// @endcond
  ...
  double xi2D_model (double, double, void *, vector<double>);
  /// @endcond

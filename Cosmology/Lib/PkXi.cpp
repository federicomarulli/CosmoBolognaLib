...
{ 
  ...
  string dir = par::DirCosmo+"Cosmology/"+code+"/";
  string dirC = par::DirCosmo+"Cosmology/CAMB/";
  if (chdir (dirC.c_str())) {};
  ...
  if (code=="CAMB" || code=="MPTbreeze-v1") {
	...
	if (lgm<lgkk[i] && lgkk[i]<lgM) {
	  interpolation_extrapolation(lgkk[i], lgkkM, lgPkM, "Linear", -1, &lgPK, &err);
	  lgPk[i] = lgPK;
	}
      }
    }
...
{ 
  ...
  if (method_Pk=="CAMB" || method_Pk=="MPTbreeze-v1" || method_Pk=="classgal_v1") {
    ...
    double err = -1, lgPK = -1;  
    interpolation_extrapolation(lgk,lgkk,lgPk,"Linear",-1,&lgPK,&err);

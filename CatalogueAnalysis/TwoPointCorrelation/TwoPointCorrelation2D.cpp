...
shared_ptr<Data> cosmobl::twopt::TwoPointCorrelation2D::correlation_NaturalEstimator (const shared_ptr<pairs::Pair> dd, const shared_ptr<pairs::Pair> rr, const int nData, const double nData_weighted, const int nRandom, const double nRandom_weighted)
{
  ...
  for (int i=0; i<dd->nbins_D1(); i++) {
	...
	if (rr->PP2D_weighted(i, j)<1.e-30) 
	  ErrorCBL("Error in correlation_NaturalEstimator() of TwoPointCorrelation2D.cpp: there are no random objects in the bin "+conv(i, par::fINT)+"; please, either increase the total number of random objects or enlarge the bin size!");
...
shared_ptr<Data> cosmobl::twopt::TwoPointCorrelation2D::correlation_LandySzalayEstimator (const shared_ptr<pairs::Pair> dd, const shared_ptr<pairs::Pair> rr, const shared_ptr<pairs::Pair> dr, const int nData, const double nData_weighted, const int nRandom, const double nRandom_weighted)
{
  vector<double> scale_D1, scale_D2;
  vector<vector<double>> xi, error;
  scale_D1.resize(m_dd->nbins_D1()); 

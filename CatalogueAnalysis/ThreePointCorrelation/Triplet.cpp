...
shared_ptr<Triplet> cosmobl::triplets::Triplet::Create (const TripletType type, const double side_s, const double side_u, const double perc_increase, const int nbins)
{
...
{
  double Lmin = (m_side_u*m_side_s-m_side_s)*(1-m_perc_increase);
  double Lmax = (m_side_u*m_side_s+m_side_s)*(1+m_perc_increase);
  m_binSize = (Lmax-Lmin)/m_nbins;

  m_scale.resize(m_nbins);
  for (int i=0; i<m_nbins; i++)
    m_scale[i] = (m_side_s)*(m_side_u-1)+((i+0.5)*m_binSize);
}
...
void cosmobl::triplets::Triplet1D_comoving_side::put (const shared_ptr<catalogue::Object> obj1, const shared_ptr<catalogue::Object> obj2, const shared_ptr<catalogue::Object> obj3)
{
  ...
  int klin = int((r23-(m_side_s*(m_side_u-1)*(1-m_perc_increase)))/m_binSize);
  //int klin = int((r23-m_side_s)/m_binSize);

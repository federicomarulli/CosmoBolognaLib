...
cbl::measure::angularpk::PowerSpectrum_angular::PowerSpectrum_angular (const catalogue::Catalogue data, const double l_min, const double l_max, const int bandwidth)
{
  m_l_min = l_min;   
  m_l_max = l_max;
...
void cbl::measure::angularpk::PowerSpectrum_angular::read_mixing_matrix (const std::string dir_matrix, const std::string file_matrix, std::vector<double> &ll, std::vector<std::vector<double>> &matrix)
{
  std::ifstream in;
...
void cbl::measure::angularpk::PowerSpectrum_angular::measure (const AngularEstimator estimator, const string dir_correlation_input, const string file_correlation_input, const int n_lines_header, CoordinateUnits inputUnits, BinType input_binType, cbl::measure::ErrorType errorType, const string dir_correlation_output, const string file_correlation_output, const int nMocks)
{
  ...
  case(AngularEstimator::_Fast_):
   ...
   case(AngularEstimator::_SphericalArmonic_):
     ...
     break;
   default:
     ErrorCBL("Invalid estimator!","cbl::measure::PowerSpectrum_angular::measure", "AngularPowerSpectrum.cpp" );
  }
...
void cbl::measure::angularpk::PowerSpectrum_angular::measureSphericalArmonic ()
{
  ...
  double survey_area=(cos(pi_2-m_catalogue->Max(cbl::catalogue::Var::_Dec_))-cos(pi_2-m_catalogue->Min(cbl::catalogue::Var::_Dec_)))*(m_catalogue->Max(cbl::catalogue::Var::_RA_)-m_catalogue->Min(cbl::catalogue::Var::_RA_));
  ...
  for(int l=m_l_min; l<m_l_max; l++){ //works only with l<=879
	...
	const int ndim = 2;
	auto integrand_real = [&l, &m] (std::vector<double> x) { return pow(-1,m)*boost::math::spherical_harmonic(l, m, x[0], x[1]).real()*sin(x[0]); };
	auto integrand_im = [&l, &m] (std::vector<double> x) { return pow(-1,m)*boost::math::spherical_harmonic(l, m, x[0], x[1]).imag()*sin(x[0]); };
	auto integrand_J_lm = [&l, &m] (std::vector<double> x) { return pow(abs(boost::math::spherical_harmonic(l, m, x[0], x[1])),2)*sin(x[0]); };
